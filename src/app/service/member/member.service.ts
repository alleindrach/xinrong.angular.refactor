...
export class MemberService {
  ...
  loginFailTooMuch$(): Observable<boolean> {
    return this.http.get<LoginFail>(environment.baseUrl + '/v2/login/get_login_fail_times.jso').pipe(
      map(
        (x: LoginFail) => {
  ...
  login$(username: string, password: string, captcha: string = null, seed: number = null): Observable<Result> {
    return this.http.get<Result>(environment.baseUrl + '/v2/login/login.jso',
      {
        params: {
          'username': username,
          'password': password,
  ...
  insession$(): Observable<Session> {
    return this.http.get<Session>(environment.baseUrl + '/v2/login/in_session_data.jso');
  }
